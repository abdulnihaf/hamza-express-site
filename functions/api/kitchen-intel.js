...
// --- Live order board: current active orders with prep status ---
async function handleLive(odooUrl, db, uid, apiKey, headers) {
  const now = new Date();
  // Look back 4 hours for active orders
  const lookback = new Date(now.getTime() - 4 * 60 * 60 * 1000);
  const fromOdoo = lookback.toISOString().slice(0, 19).replace('T', ' ');

  // Get recent orders from both POS configs
  const CONFIG_IDS = [5, 6]; // Cash Counter, Captain
  ...
  // Get prep lines and states in parallel
  const [prepLines, prepStates, orderLines] = await Promise.all([
    rpc(odooUrl, db, uid, apiKey, 'pos.prep.line', 'search_read',
      [[['order_id', 'in', orderIds]]],
      { fields: ['id', 'order_id', 'product_id', 'category_id', 'quantity', 'create_date'] }),
    rpc(odooUrl, db, uid, apiKey, 'pos.prep.state', 'search_read',
      [[['order_id', 'in', orderIds]]],
      { fields: ['id', 'order_id', 'prep_line_id', 'stage_id', 'todo', 'last_stage_change'] }),
  ...
  // Build order lines map for item details
  const olByOrder = {};
  ...
  for (const order of orders) {
    ...
    // Get prep lines for this order
    const lines = prepLines.filter(l => l.order_id?.[0] === orderId);
    const items = [];
    let allDone = true;
    ...
    for (const line of lines) {
      ...
      const catName = line.category_id?.[1] || 'Unknown';
      const catId = line.category_id?.[0] || 0;
      const productName = cleanProductName(line.product_id?.[1] || 'Unknown');

      // Find the current stage for this item across displays
      // We care about station displays (not KP Master/TV which are mirrors)
      ...
      if (currentStage !== 'Completed' && currentStage !== 'Packed' && currentStage !== 'Ready') {
        allDone = false;
      }
      if (currentStage !== 'To prepare') anyStarted = true;

      // Track station metrics
      if (!stationMetrics[catName]) {
        stationMetrics[catName] = { catId, total: 0, prepTimes: [], waitTimes: [] };
      }
      stationMetrics[catName].total += line.quantity;
      if (timing.prepDuration > 0) stationMetrics[catName].prepTimes.push(timing.prepDuration);
    ...
    // Get KP Master status for the whole order
    const kpStatus = getKPStatus(prepLines.filter(l => l.order_id?.[0] === orderId), statesByLine);
    ...
    result.push({
      ...
      source: isWaba ? 'whatsapp' : 'counter',
      table: order.table_id?.[1] || null,
      config: order.config_id?.[0] === 6 ? 'Captain' : 'Counter',
      items,
      kpStatus,
      status: allDone ? 'ready' : anyStarted ? 'preparing' : 'queued',
  ...
  for (const [name, data] of Object.entries(stationMetrics)) {
    ...
    stations[name] = {
      ...
      avgPrepMin: avgPrep,
      samplesCount: data.prepTimes.length
    };
  }
...
// --- Historical stats: station performance, KP flow, trends ---
async function handleStats(odooUrl, db, uid, apiKey, fromParam, toParam, headers) {
  ...
  for (const line of prepLines) {
    const catName = line.category_id?.[1] || 'Unknown';
    const catId = line.category_id?.[0] || 0;
    if (!stationData[catName]) stationData[catName] = { catId, items: 0, prepTimes: [], orders: new Set() };
    stationData[catName].items += line.quantity;
    stationData[catName].orders.add(line.order_id?.[0]);

    const states = statesByLine[line.id] || [];
    const timing = computeItemTiming(line.create_date, getStationStages(states));
    if (timing.prepDuration > 0) stationData[catName].prepTimes.push(timing.prepDuration);
  }
  ...
  // --- KP Master Flow (stages 75→44→74→63) ---
  // Track how long items spend in KP Master stages
  const KP_STAGES = {
    75: 'Preparing', 44: 'Ready', 74: 'Packed', 63: 'Completed'
  };
  ...
  for (const line of prepLines) {
    ...
    for (let i = 0; i < kpStates.length - 1; i++) {
      ...
      if (dur < 0 || dur > 7200) continue; // skip bad data
  ...
  const kpFlow = {
    ...
    samples: {
      preparing: kpTransitions.preparing.length,
      ready: kpTransitions.ready.length,
      packed: kpTransitions.packed.length
    }
  };
  ...
  for (const order of orders) {
    ...
    // Total order prep time: from order creation to last item completed
    const oLines = prepLines.filter(l => l.order_id?.[0] === order.id);
    let lastDone = 0;
    let itemCount = 0;
  ...
  for (const h of hourly) {
    ...
    if (data) {
      h.orders = data.orders;
      h.items = data.items;
      h.avgPrepSec = data.prepCount > 0 ? Math.round(data.totalPrepSec / data.prepCount) : 0;
    }
  }

  // --- Order timelines (last 20 for detail view) ---
  ...
  const orderTimelines = recentOrders.map(order => {
    ...
    for (const line of oLines) {
      ...
      timeline.push({
        ...
        events: events.map(e => ({
          stage: e.stage,
          timeIST: formatIST(new Date(e.time)),
          offsetSec: orderDate ? Math.round((e.time - orderDate.getTime()) / 1000) : 0
        }))
      });
    }
    ...
    return {
      id: order.id,
      name: order.name,
      time: orderDate ? formatIST(orderDate) : '',
      type: sittingMode === 0 ? 'dine-in' : 'takeaway',
      config: order.config_id?.[0] === 6 ? 'Captain' : 'Counter',
      amount: Math.round(order.amount_total),
      timeline
...
// Station display stage IDs (station-specific KDS, not KP Master/TV mirrors)
const STATION_STAGE_IDS = new Set([
  // Indian station (Display 11): 80 To prepare, 81 Preparing, 83 Prepared
  80, 81, 83,
  // Chinese station (Display 12): 85 To prepare, 86 Preparing, 84 Prepared
  85, 86, 84,
  // Tandoor station (Display 13): 87 To prepare, 88 Preparing, 89 Prepared
  87, 88, 89,
  // FC station (Display 14): 90 To prepare, 91 Preparing, 92 Prepared
  90, 91, 92,
  // Juice (Display 16): 45 To prepare, 46 Preparing, 47 Ready, 48 Completed
  45, 46, 47, 48,
  // BM (Display 17): 49 To prepare, 58 Preparing, 50 Packed, 51 Completed
  49, 58, 50, 51,
  // Shawarma (Display 18): 52 To prepare, 59 Preparing, 53 Ready, 54 Completed
  52, 59, 53, 54,
  // Grill (Display 19): 55 To prepare, 60 Preparing, 56 Ready, 57 Completed
  55, 60, 56, 57
]);

// Map stage IDs to human-readable names and sequence
const STAGE_INFO = {
  // Indian
...
function getCurrentStage(stationStages) {
  ...
  // The stage where todo=false with highest sequence = completed stage
  // The stage where todo=true with lowest sequence = current stage
  let highestDone = -1;
  let highestDoneName = 'To prepare';
  for (const s of stationStages) {
    const info = STAGE_INFO[s.stage_id?.[0]];
    if (!info) continue;
    if (!s.todo && info.seq > highestDone) {
      highestDone = info.seq;
      highestDoneName = info.name;
    }
  }
  // If highest done stage is "Prepared"/"Ready"/"Packed"/"Completed", item is done
...
function getKPStatus(orderLines, statesByLine) {
  // Check KP Master stages (75, 44, 74, 63) across all items
...
function computeItemTiming(createDate, stationStages) {
  ...
  const now = Date.now();
  const elapsed = created ? Math.round((now - created.getTime()) / 1000) : 0;

  // Prep duration: from create_date to the "Prepared"/"Ready"/"Packed" stage
  let prepDuration = 0;
  for (const s of stationStages) {
    const info = STAGE_INFO[s.stage_id?.[0]];
    if (!info) continue;
    if (['Prepared', 'Ready', 'Packed'].includes(info.name) && !s.todo) {
      ...
      if (doneTime && created) {
        prepDuration = Math.round((doneTime - created.getTime()) / 1000);
      }
    }
  }
...
function parseSittingMode(json) {
  if (!json) return 2; // default takeaway
  try {
    const parsed = JSON.parse(json);
    // Could be an object or array of changes
    if (typeof parsed === 'object' && !Array.isArray(parsed)) {
      return parsed.sittingMode ?? 2;
    }
    if (Array.isArray(parsed) && parsed.length > 0) {
      const last = parsed[parsed.length - 1];
      return last.sittingMode ?? 2;
    }
  } catch { /* ignore */ }
  return 2;
}
...
function emptySummary() {
  return { totalOrders: 0, avgOrderPrepSec: 0, medianOrderPrepSec: 0, p90OrderPrepSec: 0, counterOrders: 0, captainOrders: 0, totalItems: 0 };
}
